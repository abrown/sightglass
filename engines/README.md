...
The `_`-prefixed pairs are ignored for now__*__, but the other pairs correspond to `ARG` statements
...
> __*__ the `_COMMIT` setting is actually used for overwriting the revision to avoid situations
> where, e.g., a branch is used and additional commits are added between builds. Also, `_RUSTC`
> could be used to alter the compiler version.
