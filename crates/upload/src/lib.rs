...
pub fn upload(server: &str, dry_run: bool, measurements: &Vec<Measurement>) -> Result<()> {
    ...
    for engine_path in found_engines.into_iter() {
        ...
        let engine_id = database.create_if_not_exists("engines", &engine, &engine.id)?;
    ...
    for benchmark_path in found_benchmarks.into_iter() {
        ...
        let benchmark_id =
            database.create_if_not_exists("benchmarks", &benchmark, &benchmark.id)?;
        log::debug!(
            "Mapping benchmark: {} -> {}",
            &benchmark_path,
            &benchmark_id
    ...
    let machine = database.create_if_not_exists("machines", &machine, &machine.id)?;
